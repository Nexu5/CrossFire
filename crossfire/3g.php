...
exec("echo /www/crossfire/3g/3g.sh | at now");}

if (isset($_GET[enablekeepalive])) {
	if (exec("grep 3g-keepalive.sh /etc/crontabs/root") == "") {
		exec("echo '*/5 * * * * /www/crossfire/3g/3g-keepalive.sh' >> /etc/crontabs/root");
...
if (isset($_GET[enable])) {
	...
	if (exec("grep 3g.sh /etc/rc.local") == "") {
		...
		exec("echo /www/crossfire/3g/3g.sh >> /etc/rc.local");
...
<?php
$filename = "/var/www/crossfire/3g/3g.sh";
...
<input type='hidden' name='filename' value='/var/www/crossfire/3g/3g.sh'>
