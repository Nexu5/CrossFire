...
if (isset($_GET[connect])) {
	if (exec("ps aux | grep [s]sh | grep -v -e ssh.php | grep -v grep") == "") {
		...
		exec("echo /www/CrossFire/ssh/ssh-connect.sh | at now");
		sleep(2);
	} else {
...
if (isset($_GET[enablekeepalive])) {
	if (exec("grep ssh-keepalive.sh /etc/crontabs/root") == "") {
		exec("echo '*/5 * * * * /www/CrossFire/ssh/ssh-keepalive.sh' >> /etc/crontabs/root");
...
if (isset($_GET[enable])) {
	if (exec("grep ssh-connect.sh /etc/rc.local") == "") {
		...
		exec("echo /www/CrossFire/ssh/ssh-connect.sh >> /etc/rc.local");
...
$filename = "/www/CrossFire/ssh/ssh-connect.sh";
  $fh = fopen($filename, "r") or die("Could not open file! Empty?");
  $data = fread($fh, filesize($filename)) or die("Could not read file!");
  fclose($fh);
 echo "<b>SSH Connection Command:</b><form action='$_SERVER[php_self]' method= 'post' ><input type='hidden' name='filename' value='/www/CrossFire/ssh/ssh-connect.sh'>
